...
function ChangePassword() {
    ...
    function changePasswordClick() {
        ...
        if (passwordError.innerText === "" && confirmPasswordError.innerText === '' && passwordResetCodeError.innerText === '') {
            axios.get(`https://password-reset1-qaqa.onrender.com/getUser/${userEmail}`)
                .then((response) => {
                    if (response.data.data[0].randomString === passwordResetCode.value) {
                        ...
                        axios.put(`https://password-reset1-qaqa.onrender.com/changePassword/${userEmail}`, newPassword)
                            .then((res) => {
                                setOpen(true)
                                setTimeout(() => {
